...
export async function writeRpcsJson() {
  if (!existsSync('out')) {
    mkdirSync('out')
  }

  const rpcs = await generateChainData()
  writeFileSync("out/rpcs.json", JSON.stringify(rpcs, null, 2))
}


(async () => {
  const rpcs = readFileSync('out/rpcs.json').toString()
  const rpcJson = JSON.parse(rpcs)
  const dirs = readdirSync('png')
  const total = []
  const coinObj = _.values(cc_coins)
  const coins = _.values(cc_coins)
  _.forEach(rpcJson, (v) => {
    const data = _.filter(coins, x => x.name === v.name || _.get(v, 'nativeCurrency.symbol') == x.symbol || v.shortName === x.name)
    if (!_.isEmpty(data)) {
      total.push(v);
    }
  })
